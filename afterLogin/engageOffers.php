...
<body>
    ...
    <nav id="siteNav" class="navbar navbar-default navbar-fixed-top" role="navigation">
        ...
        <?php   
        $jobOwnerEmail=$_GET['at'];
        $jobBidderEmail=$_SESSION['email'];
        $jobID=$_GET['key'];
        ...
        ?>
    <div>
        <h2>Give an Offer for this Job</h2>
        <label>Job Owner <?php  echo $jobOwnerEmail;  ?></label><br>
        <label> Job Category: <?php echo $row['jobCategory']; ?></label><br>
        <label>Job Description: <?php echo $row['jobDescription']; ?></label><br>
        <label>ago: <?php echo get_time_ago($row["timeAgo"]); ?></label>
    </div>
<form method="POST"><br>
    <label>Bid with an offer</label><br>
    <input type="text" name="deep" placeholder="Write some description for an offer" required><br>
    <input type="number" name="offerValue" placeholder="Offer in ETB" required><br>
    <input type="Submit" name="submitOffer" value="Submit Offer">
</form>

<?php
if (isset($_POST['submitOffer'])) {
    $offeredBudget=$_POST['offerValue'];
    $offeredDeep=$_POST['deep'];
    $jobCategory=$row['jobCategory'];
    $jobDescription=$row['jobDescription'];
    $jobBudget=$row['jobBudget'];
    $jobDeadline=$row['jobDeadline'];
    $timeAgo=time();
    $jobShow=0;

    // OfferID jobID   senderEmail receiverEmail   jobCategory jobDescription  jobDeadline jobBudget   timeAgo jobShow 
    $sqlRequestPre="SELECT * FROM offerslist WHERE jobID='$jobID' ";
        $resultRequestPre=mysqli_query($con, $sqlRequestPre);
            if (mysqli_num_rows($resultRequestPre) > 0) {
              while($rowRequestPre = mysqli_fetch_assoc($resultRequestPre)) {
                if($rowRequestPre['jobShow']==0 AND $rowRequestPre['senderEmail']==$jobBidderEmail AND $rowRequestPre['receiverEmail']=$jobOwnerEmail){
                    $sqlRequest="UPDATE offerslist SET jobBudget= '$jobBudget',offeringPrice= '$offeredBudget', timeAgo='$timeAgo' WHERE jobID='$jobID' AND senderEmail='$jobBidderEmail' AND receiverEmail='$jobOwnerEmail' ";
              ...
              $sqlRequest="INSERT INTO offerslist (jobID, senderEmail, receiverEmail, jobCategory, jobDescription, jobBudget, jobDeadline, timeAgo, jobShow,offeringPrice) VALUES('$jobID', '$jobBidderEmail', '$jobOwnerEmail', '$jobCategory', '$jobDescription', '$jobBudget', '$jobDeadline', '$timeAgo', '$jobShow', '$offeredBudget')";
                    if(mysqli_query($con, $sqlRequest)){
                            echo "Success";
                        }
            }
